...
class AutoFillManager {
  constructor() {
    ...
    this.STRIPE_SELECTORS = {
      ...
      CARD_MONTH: '#cardMonth, #expiryMonth, [name*="month"], [placeholder*="MM"]',
      CARD_YEAR: '#cardYear, #expiryYear, [name*="year"], [placeholder*="YY"], [placeholder*="YYYY"]',
      CARD_CVC: '#cardCvc, #cvc, #cvv, [name*="cvc"], [name*="cvv"], [placeholder*="CVC"], [placeholder*="CVV"], [placeholder*="Security"]',
      BILLING_NAME: '#billingName',
      BILLING_COUNTRY: '#billingCountry',
      BILLING_ADDRESS: '#billingAddressLine1',
      BILLING_POSTAL: '#billingPostalCode', 
      BILLING_CITY: '#billingLocality',
      SUBMIT_BUTTON: 'button[type="submit"], button.SubmitButton, button[data-testid="hosted-payment-submit-button"], [role="button"][data-testid*="submit"], .SubmitButton--complete, .SubmitButton--pre, .SubmitButton--processing, button[class*="SubmitButton"]',
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      switch (message.type) {
        case "fillPaymentForm":
          this.fillPaymentForm(message.data).then(() => {
        ...
        case "startProTrialActivation":
          this.startProTrialActivation(message.cards).then(() => {
            sendResponse({ success: true });
          }).catch(error => {
  ...
  detectPaymentForms() {
    ...
    for (const button of buttons) {
      ...
      if ((text.includes('start') && text.includes('trial')) || 
      ...
      const response = await new Promise((resolve, reject) => {
        chrome.runtime.sendMessage({
          ...
          if (chrome.runtime.lastError) {
            ...
            reject(new Error(response.error || 'Failed to generate payment data'));
          } else {
            resolve(response);
          }
        });
  ...
  async findAndClickTrialButton() {
    ...
    for (const selector of buttonSelectors) {
      try {
        ...
        console.log(`🔍 Checking ${buttons.length} buttons with selector: ${selector}`);
        
        for (const button of buttons) {
          const buttonText = button.textContent?.toLowerCase()?.trim() || '';
          
          // Check for trial-related text
          if ((buttonText.includes('start') && (buttonText.includes('trial') || buttonText.includes('14'))) ||
              (buttonText.includes('continue') && window.location.pathname.includes('/trial'))) {
  ...
  async fillStripeForm(data) {
    ...
    if (this.formFilledSuccessfully && this.waitingForStripeResponse) {
      console.log('⚠️ Form already filled successfully, waiting for Stripe response...');
      return true;
    }
    
    console.log('💳 Starting Stripe form fill...');
    console.log('📋 Using data:', {
      cardCount: data.cards?.lengthё,
      name: data.name,
    ...
    try {
      ...
      // Fill card number
      const cardNumberField = document.querySelector(this.STRIPE_SELECTORS.CARD_NUMBER);
      if (cardNumberField) {
        const success1 = await this.fillFieldReliably(cardNumberField, currentCard.number, 'Card Number');
      ...
      if (submitResult) {
        console.log('✅ Form submitted successfully, monitoring for response...');
      ...
      this.showTrialStatus(`❌ Failed to fill payment form: ${error.message}`, 'error');
      return false;
    }
  }
  ...
  async waitForCardInputs() {
    ...
    const cardToggle = document.querySelector(this.STRIPE_SELECTORS.CARD_SECTION_TOGGLE);
    if (cardToggle) {
    ...
    for (let i = 0; i < 30; i++) {
      const cardNumber = document.querySelector(this.STRIPE_SELECTORS.CARD_NUMBER);
      const cardExpiry = document.querySelector(this.STRIPE_SELECTORS.CARD_EXPIRY);
      ...
      const hasExpiryField = (cardExpiry && this.isFieldReady(cardExpiry)) || 
                           (cardMonth && cardYear && this.isFieldReady(cardMonth) && this.isFieldReady(cardYear));
  ...
  isFieldReady(field) {
    ...
    return field.offsetWidth > 0 && field.offsetHeight > 0 && !field.disabled && !field.readOnly;
  }
  ...
  async fillFieldReliably(field, value, fieldName) {
    ...
    const isStripeCheckoutInput = field.classList.contains('CheckoutInput') || 
    ...
    if (fieldName.includes('Card Number')) {
      ...
      console.log(`⚠️ ${fieldName} partially filled. Expected: "${cleanValue}", Got: "${field.value.replace(/\s/g, '')}"`);
    ...
    console.log(`🔄 ${fieldName} Method 1 failed, trying character-by-character...`);
    field.value = '';
    field.dispatchEvent(new Event('input', { bubbles: true }));
    ...
    console.log(`❌ ${fieldName} failed to fill with all methods. Final value: "${field.value}"`);
    return false;
  }
  ...
  async fillExpiryFields(card) {
    ...
    const cardExpiryField = document.querySelector(this.STRIPE_SELECTORS.CARD_EXPIRY);
    if (cardExpiryField && this.isFieldReady(cardExpiryField)) {
      const expiryValue = `${card.month.toString().padStart(2, '0')} / ${card.year}`;
      const success = await this.fillFieldReliably(cardExpiryField, expiryValue, 'Card Expiry (Combined)');
      if (success) return true;
      
      // Try alternative format
      const altValue = `${card.month.toString().padStart(2, '0')}${card.year}`;
      const success2 = await this.fillFieldReliably(cardExpiryField, altValue, 'Card Expiry (MMYY)');
      if (success2) return true;
    }
    
    // Try separate month and year fields
    const cardMonthField = document.querySelector(this.STRIPE_SELECTORS.CARD_MONTH);
    const cardYearField = document.querySelector(this.STRIPE_SELECTORS.CARD_YEAR);
    
    if (cardMonthField && cardYearField) {
      console.log('📅 Trying separate month/year fields...');
      
      const monthSuccess = await this.fillFieldReliably(cardMonthField, card.month.toString().padStart(2, '0'), 'Card Month');
      await this.wait(200);
      ...
      yearSuccess = await this.fillFieldReliably(cardYearField, yearValue2, 'Card Year (YY)');
      if (!yearSuccess) {
        yearSuccess = await this.fillFieldReliably(cardYearField, yearValue4, 'Card Year (YYYY)');
      }
  ...
  async fillCvcField(cvv) {
    ...
    for (const selector of cvcSelectors) {
      ...
      if (cvcField && this.isFieldReady(cvcField)) {
        ...
        const success = await this.fillFieldReliably(cvcField, cvv, `CVC (${selector})`);
        if (success) return true;
      }
    }
  ...
  async fillBillingInfo(data) {
    ...
    // Fill name
    const nameField = document.querySelector(this.STRIPE_SELECTORS.BILLING_NAME);
    if (nameField) {
      await this.fillFieldReliably(nameField, data.name, 'Billing Name');
    }
    
    // Fill country (special handling for select)
    const countryField = document.querySelector(this.STRIPE_SELECTORS.BILLING_COUNTRY);
    if (countryField) {
    ...
    // Fill address
    const addressField = document.querySelector(this.STRIPE_SELECTORS.BILLING_ADDRESS);
    if (addressField) {
      await this.fillFieldReliably(addressField, data.address.street, 'Billing Address');
    }
    
    // Fill postal code
    const postalField = document.querySelector(this.STRIPE_SELECTORS.BILLING_POSTAL);
    if (postalField) {
      await this.fillFieldReliably(postalField, data.address.postalCode, 'Postal Code');
    }
    
    // Fill city
    const cityField = document.querySelector(this.STRIPE_SELECTORS.BILLING_CITY);
    if (cityField) {
  ...
  async validateForm() {
    ...
    const cardYear = document.querySelector(this.STRIPE_SELECTORS.CARD_YEAR);
    const cardCvc = document.querySelector(this.STRIPE_SELECTORS.CARD_CVC);
    const billingName = document.querySelector(this.STRIPE_SELECTORS.BILLING_NAME);
    
    console.log('📋 Field validation details:');
    
    // Check required card fields
    const cardNumberValue = cardNumber?.value?.replace(/\s/g, '') || '';
    console.log(`  - Card Number: "${cardNumber?.value}" (clean: "${cardNumberValue}", length: ${cardNumberValue.length})`);
    if (!cardNumberValue || cardNumberValue.length < 16) {
      console.log(`❌ Card number invalid: needs 16 digits, got ${cardNumberValue.length}`);
      return false;
    }
    
    // Check billing name
    console.log(`  - Billing Name: "${billingName?.value || 'NOT FOUND'}" (length: ${billingName?.value?.length || 0})`);
    ...
    console.log(`  - Card Expiry Combined: "${cardExpiry?.value || 'NOT FOUND'}"`);
    ...
    console.log(`  - Card CVC: "${cvcField?.value || 'NOT FOUND'}" (length: ${cvcField?.value?.length || 0})`);
    if (!cvcField?.value || cvcField.value.length < 3) {
      console.log(`❌ Card CVC invalid: needs 3+ digits, got ${cvcField?.value?.length || 0}`);
      return false;
    }
    ...
    if (errorElements.length > 0) {
      ...
      errorElements.forEach((element, index) => {
        console.log(`  - Error ${index + 1}: ${element.id || element.name || element.className} = "${element.value}"`);
      });
      return false;
    }
  ...
  async submitFormImmediately() {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (submitButton) {
        ...
        const canClick = !submitButton.disabled && 
    ...
    for (const button of allButtons) {
      ...
      if (text.includes('submit') || text.includes('pay') || text.includes('continue') || 
          text.includes('confirm') || text.includes('next') || 
          value.includes('submit') || value.includes('pay')) {
    ...
    const checkForErrors = () => {
      ...
      for (const selector of errorSelectors) {
        ...
        if (errorElement && errorElement.textContent.trim()) {
          console.log(`❌ Stripe error detected: ${errorElement.textContent.trim()}`);
          hasError = true;
          break;
        }
      }
      ...
      const submitButton = document.querySelector(this.STRIPE_SELECTORS.SUBMIT_BUTTON);
      const buttonAvailableAgain = submitButton && !submitButton.disabled && 
                                  submitButton.getAttribute('aria-disabled') !== 'true';
      
      if (hasError || (buttonAvailableAgain && Date.now() - this.lastSubmitTime > 3000)) {
        if (hasError) {
          console.log('🔄 Error detected, resetting form state for retry...');
        } else {
          console.log('🔄 Submit button available again, likely an error occurred...');
        }
  ...
  async startProTrialActivation(cards) {
    ...
    try {
      console.log(`🚀 Starting Pro Trial activation with ${cards.length} cards...`);
  ...
  async tryCardsSequentially() {
    while (this.cardIndex < this.cards.length && this.isActivatingTrial) {
      console.log(`🔄 Trying card ${this.cardIndex + 1}/${this.cards.length}...`);
      ...
      if (success) {
        console.log('✅ Form filled successfully, waiting for Stripe response...');
        ...
        if (responseReceived === 'success') {
          ...
          console.log(`❌ Card ${this.cardIndex + 1} failed, trying next card...`);
          this.cardIndex++;
  ...
  async fillGenericForm(data) {
    ...
    for (const selector of cardNumberSelectors) {
      ...
      if (field && this.isFieldReady(field)) {
        await this.fillFieldReliably(field, currentCard.number, 'Card Number (Generic)');
        break;
      }
    }
  ...
  initAutoActivation() {
    ...
    if (this.formFilledSuccessfully) {
      console.log('⚠️ Form already filled successfully, skipping auto-activation');
      return;
    }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.href.includes('checkout.stripe.com') && window.autoFillManager.generatedData) {
      setTimeout(() => {
        // Only fill if not already filled
        if (!window.autoFillManager.formFilledSuccessfully) {
          window.autoFillManager.fillStripeForm(window.autoFillManager.generatedData);
        } else {
